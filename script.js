...
function changeColor(e){
  ...
  if(this.classList.contains('colored')){
    let nowColor = this.style.backgroundColor;
    let newColor = pSBC(-.1, nowColor);
    ...
    if(useRandom){
      ...
      //use the selected color by user
    }
    //changes backgroundColor when clicked on a div
  }
}

function btnClicked(e){
  const gridSize = sizeSelector.value;
